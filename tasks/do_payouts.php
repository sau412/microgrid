...
$payout_data = db_query_to_array("SELECT `uid`, `balance`, `withdraw_address` FROM `users`
									WHERE `balance` <> 0 AND `withdraw_address` <> ''");
...
foreach($payout_data as $payout_row) {
	...
	// Add payout
	db_query("INSERT INTO `payouts` (`user_uid`, `address`, `amount`)
					VALUES ('$user_uid_escaped', '$address_escaped', '$balance_escaped')");
